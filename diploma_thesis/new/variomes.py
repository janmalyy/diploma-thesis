...
class VariomesProvider:
    def __init__(self):
        self.base_url = "https://variomes.sibils.org/api/rankLit"

    def fetch_data(self, variant: Variant) -> list[Article]:
        """
        Fetches data from Variomes API for a given variant and returns a list of Article objects.
        """
        try:
            r = requests.get(url=f"{self.base_url}?genvars={variant.variant_string}")
            r.raise_for_status()
            data = r.json()
        ...
        if supp_list and variant.terms:
            ...
            for supp in supp_list:
                ...
                article.suppl_info = self._process_suppl_data(supp, pattern)

        return articles

    def _process_suppl_data(self, data: dict, pattern: re.Pattern) -> str:
        """
        Extracts snippets from supplemental data based on variant terms.
        """
